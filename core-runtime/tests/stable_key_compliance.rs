...
fn test_stable_key_format_compliance() -> anyhow::Result<()> {
    ...
    for key in keys {
        assert!(re.is_match(&key), "Key '{}' does not match SHA-256 hex format", key);
    }
    Ok(())
}
...
fn test_ambiguous_flag_on_collision() -> anyhow::Result<()> {
    ...
    assert!(btn2.ambiguous, "Second occurrence (collision) should be ambiguous");
