...
## 9. ポジショニング更新ノート (2026-06-27)

本ノートは `docs/bench-playwright-comparison.md` の実測結果を受けて、製品訴求の重点を以下の通り更新する。**SPEC 本文（§1〜§8）は設計思想として変更しない。**

### 9.1 訴求の修正

| 旧訴求 | 実態 | 更新後の訴求 |
|---|---|---|
| 「LLM 入力トークンを平均 90% 削減」 | 初回コールは Playwright custom extract 比 3〜4.5× 大きい | **「継続操作の累積コストを削減」**（2 回目以降の delta 配信が主効果） |
| 「Raw HTML 比のトークン削減」 | 単純ページ（example.com）は -60%、複雑ページは逆転 | **条件付きで正確**。対象ページタイプを明示する |

### 9.2 実測から見えた本質的優位性（優先度順）

1. **セレクタ安定性（stable_key）**: UI リファクタリングを経ても同一要素を再特定。長期稼働エージェントで決定的な差。
2. **デルタ配信（RFC 6902 JSON Patch）**: 2 回目以降のコールは典型 20〜50 tokens。マルチステップワークフローで累積コストが逆転する。
3. **ポリシーエンジン + HITL**: 金融取引・高リスクアクションの自動ブロックと人間承認。Playwright には相当機能がない。
4. **プロンプトインジェクション検出**: `security_flags` による構造化リスク公開。
5. **監査ログ + PII マスク**: エンタープライズ・コンプライアンス要件への対応。

### 9.3 今後の計測課題

- **デルタ配信の実測**: 10 ステップ操作の累積トークン比較（Playwright vs DH）
- **stable_key 短縮の効果測定**: 先頭 16 文字化による初回ペイロード削減量
- **ナビ/フッターフィルタリング**: デフォルト除外時のトークン削減効果
