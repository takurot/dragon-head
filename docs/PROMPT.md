...
実装に入る前に実装計画を立案します。コーディングを始めるのはプランが確定してからです。
...
ドラフトプランができたら、`Agent` ツールを使って以下の観点から**並列に**レビューを依頼し、プランをブラッシュアップする（1メッセージで複数の `Agent` 呼び出しを行う）。各エージェントには、ドラフトプラン全文・`PLAN_TASK`・関連する `docs/PLAN.md` / `docs/SPEC.md` のセクション・(あれば) Issue 内容を渡す。

| レビュー観点 | 推奨サブエージェント | チェック内容 |
|---|---|---|
| 仕様・Exit Criteria 整合性 | `architect` または `planner` | `docs/SPEC.md` / `docs/PLAN.md` との整合、ステップ分解の妥当性、Exit Criteria の網羅性 |
| テスト戦略 | `tdd-guide` | Unit / Integration / E2E の配置・粒度、`docs/testing.md` との整合、カバレッジ目標(80%)達成可否 |
| Trust Boundary / セキュリティ | `security-reviewer` | Policy / Audit / SessionVault / Plugin / MCP などの trust boundary への影響、入力検証、秘密情報の扱い |
| リスク・依存関係 | `architect` | 外部 crate 依存、breaking change、crate 境界の崩れ、scope creep の有無 |
...
- [ ] 不明点・前提条件の曖昧さがない（あれば一次情報（コード・docs・Issue）を調査して解消する。それでも解決不能な場合のみユーザーに報告する）
